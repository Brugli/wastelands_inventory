...
export(GameEnums.EQUPTMENT_TYPE) var type

func _ready():
	placeholder.texture = ItemManager.get_placeholder(type)

func try_put_item(new_item: Item) -> bool:
	return (new_item.equiptment_type == type || new_item.secondary_type == type) and .try_put_item(new_item)
	
func put_item(new_item:Item) -> Item:
	if new_item:
		if new_item.equiptment_type != type and new_item.secondary_type != type:
			return new_item
		placeholder.hide()
	else:
		placeholder.show()
